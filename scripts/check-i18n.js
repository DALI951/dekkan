...
// Verifies: index.html data-i18n/-ph/-title keys, js/app.js T.t('...') keys,
...
for (const m of app.matchAll(/T\.t\('([^']+)'\)/g)) appKeys.add(m[1]);
...
appKeys.forEach(function (k) { if (!(k in ar)) report("app.js uses '" + k + "' — not in the dicts"); });

console.log('i18n keys: AR=' + arKeys.length + ' EN=' + enKeys.length +
  ' | html uses ' + htmlKeys.size + ' | app.js uses ' + appKeys.size);
